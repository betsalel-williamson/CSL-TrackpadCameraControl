...
{
    /// <summary>
    /// Builds op-section body copy: gesture bindings (style table) + live Cities keymapping labels.
    /// Options uses <see cref="OpDescriptionZoom"/> etc.; Debug may use OpHeading* (title + body).
    /// </summary>
    internal static partial class VanillaCameraKeyLabels
    {
        ...
        /// <summary>Gesture line from a settings binding pair.</summary>
        ...
        /// <summary>Gesture line for an op from the live style table.</summary>
        ...
        /// <summary>
        /// Options body: Gesture(s) + Keymapping(s) only. Group title is separate;
        /// meaning stays out so the section stays compact.
        /// </summary>
        private static string BuildDescription(string gestureLine, string keymappingBindings)
        {
        ...
        private static ModSettings LabelSettings()
        {
            try
            {
                return Mod.Settings ?? ModSettings.CreateFactoryDefaults();
            }
            catch
            {
