...
Shipped capture is **AppleKit** with [Maps+](../glossary/maps-plus-preset.md) gesture style. Tune [Sensitivity](../glossary/sensitivity.md) and [feel presets](./feel-presets.md) from Options; CAD, Contacts, and Assist chrome stay off the product surface unless their flags are on.
...
4. With the game focused, try two-finger drag (pan), pinch (zoom), two-finger rotate (yaw), and Option (`⌥`)+two-finger drag (orbit). Trackpad pan should not also vanilla-zoom; a real mouse wheel should still zoom.
5. Open Options → Trackpad Camera Control to adjust Sensitivity, reverse, Pitch min / max, or Slow / Default / Fast.
