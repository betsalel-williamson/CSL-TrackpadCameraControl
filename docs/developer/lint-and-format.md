...
**C# pin:** in-game mod is **net35** + **LangVersion 9**; TrackpadCapture is **netstandard2.0** + **LangVersion 9** — see [contributor setup](./about-this-guide.md).

## Prerequisites

Use the automated bootstrap when possible — see [contributor setup](./about-this-guide.md):
...
| Event            | Gates                                                                                                            |
| ---------------- | ---------------------------------------------------------------------------------------------------------------- |
| **Pull request** | **Commitlint** (Ubuntu) + **Validate** (macOS: docs / format / `dotnet test`, path-scoped; tooling → full suite) |
| **Push to main** | **Release** only (Changesets version PR or tag) — CI does not re-run Validate after squash-merge                 |
...
Locally, husky **pre-commit** formats staged files; **pre-push** always runs `docs`, and on `main` also `format:check` — see [commits and releases](./release-process.md).
