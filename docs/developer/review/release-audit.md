...
Specialist pass for dev vs release version policy, MSBuild `BuildInfo`, and alignment with root Changesets workflow. Scope: `rewrite/mod/Host/Mod.cs`, `TrackpadCameraControl.Rewrite.csproj`, root `docs/developer/release-process.md`, shipping `ModBuildInfoTests` parity.

**Related:** [organized product feedback](./v1-product-feedback.md) (F7), [v1 audit plan](./v1-audit-plan.md) phase R5.

## Strengths

- **Single product semver source.** `TrackpadCameraControl.Rewrite.csproj` reads `package.json` `"version"` into `Version`, `FileVersion`, `InformationalVersion`, and generated `BuildInfo.ProductVersion` — same model as shipping and root [commits and releases](.././release-process.md).
...
- **Display APIs are split by surface.** `OptionsTitle` / `IUserMod.Name` → product semver; `DebugPanelTitle` → assembly identity; footer → UTC timestamp in clipboard, local time in panel — matches root [mod reload during development](../local-mvp-install.md) table.
...
- **Rewrite-specific release shard missing.** Root `release-process.md` documents the monorepo workflow; there is no `rewrite/docs/developer/release-process.md` for rewrite folder name, deploy path (`TrackpadCameraControl.Rewrite`), or beta checklist — called out in plan R5.2.
...
| **R6.2**    | Doc alignment with root release docs                     | Cross-link from `release-process.md` to `docs/developer/release-process.md`              |
