...
| F7  | **Version / release process** | Dev builds expose build identity; **releases** expose semver only (major.minor.patch) | `BuildInfo`, `GetAssemblyIdentityDisplay`, Debug footer shows UTC timestamp; process partially documented in root `docs/developer/release-process.md` but rewrite needs explicit shard | L12 — contract clarity                         |
