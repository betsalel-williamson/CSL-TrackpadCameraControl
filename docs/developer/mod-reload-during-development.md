...
4. If **Show debug panel** is on, the floating **Debug** panel comes back after reload (see [Debug UI across auto-reload](#debug-ui-across-auto-reload)). Footer **Built (UTC)** and **asm** must change after each rebuild; **⎘** copies build info (optionally with OS and input devices when **Include system info** is checked).
