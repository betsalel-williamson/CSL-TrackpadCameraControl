...
4. Footer line: **Built (UTC)** and **asm** identity — confirm the latest compile loaded after auto-reload; **Copy** pastes build info to the clipboard. **Include system info** (default on, on the same footer row as Copy) adds OS + Mac model, connected keyboard/mouse/trackpad **models** (VID:PID / transport; duplicates as ×N; never serials), and dependency-critical **assembly** versions (Unity, Harmony, game, this mod) for QA. Both the Copy checkbox and title-bar dismiss state **persist across sessions** (schema 4). The panel height follows content and is clamped into the game UI view so the footer stays visible.
