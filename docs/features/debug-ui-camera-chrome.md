...
1. Window title: mod name + **assembly identity** (compile stamp for reload QA). Options / Content Manager keep product semver. The title bar is the drag handle and hosts native Cities circular **close** and **gear** (Options) controls. Header buttons are translucent at rest and stronger on hover. While the pointer is over the panel, the body and title bar are fully opaque; moving off softens both so the city shows through. Gear opens **Options** focused on **Trackpad Camera Control**. Closing the panel (while Options still allows Debug) leaves a floating **Debug** reopen chip.
2. Feel preset row (Slow / Default / Fast / **New Preset** when dirty; Load on select; Reset, Save as…, and Delete).
3. Sections in order **General → Zoom → Pan → Rotate → Orbit**. Label + Sensitivity control share one row; each op section also exposes a **Deadband** threshold (Debug-only tuning for gesture activation).
4. Footer: **Include system info** then **Copy** on one row; **Built (local)** on the next (single line). Clipboard paste leads with **TrackpadCameraControl:** assembly identity, then **Built (UTC)** (no product `Mod:` line). **Include system info** (default on) adds OS + Mac model, connected keyboard/mouse/trackpad **models** (VID:PID / transport; duplicates as ×N; never serials), and dependency assemblies (Unity, Harmony, game) — this mod is not repeated under Assemblies. Both the Copy checkbox and title-bar dismiss state **persist across sessions** (schema 4). The panel height follows content and is clamped into the game UI view so the footer stays visible.
...
- With Show debug panel on, the floating panel shows feel presets and tunables while a city is loaded; title bar drag moves the whole panel 1:1 with the cursor; native circular close and gear appear in the title bar (translucent at rest, stronger on hover); hovering the panel shows it at full opacity; moving off softens it so the city shows through; gear opens Options focused on Trackpad Camera Control.
