...
4. Footer line: **Built (UTC)** and **asm** identity — confirm the latest compile loaded after auto-reload; **Copy** pastes build info to the clipboard. **Include system info (OS, devices)** (default on) adds OS, hardware, and connected keyboard/mouse/trackpad **models** (VID:PID / transport; duplicates as ×N; never serial numbers) for QA.
