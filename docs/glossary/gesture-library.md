...
The **Unity-reusable** middle stack layer: shared primitive/frame contract, backend interface, and OS→frame mappers. Lives under `src/TrackpadCameraControl.Gestures`. Any Unity title could consume it without bringing Cities: Skylines Options, Maps+ seeds, or Harmony.
