...
{
    /// <summary>
    /// In-process AppKit local monitor → GestureFrame. Fail soft when AppKit is missing
    /// (non-macOS CI / headless). Optional capture gate and precise-scroll callbacks keep
    /// game policy out of this type.
    /// </summary>
    public sealed class AppleGestureSource : IGestureSource, IDisposable
    {
        ...
        /// <summary>When set, frames enqueue only while this returns true.</summary>
        public Func<bool> ShouldCapture { get; set; }

        /// <summary>Fired for scroll-wheel events with the precise-deltas flag.</summary>
        public Action<bool> PreciseScrollChanged { get; set; }
        ...
        private IntPtr OnBlock(IntPtr block, IntPtr nsEvent)
        {
            ...
            try
            {
                ...
                if (type == AppleGestureMapper.EventTypeScrollWheel)
                {
                    ...
                    precise = objc_msgSend_bool(
                        nsEvent,
                        sel_registerName("hasPreciseScrollingDeltas")
                    );
                    Action<bool> onPrecise = PreciseScrollChanged;
                    if (onPrecise != null)
                    {
                        onPrecise(precise);
                    }
            ...
            {
                // fail soft — never drop the NSEvent
            }
